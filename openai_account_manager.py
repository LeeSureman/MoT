...
class OpenAI_Account_Manager_MultiThread_One_Acount_Many_Used:
    ...
    def __init__(self, used_account_fp, all_account_fp, limit_account_num=-1):
    ...
    def get_next_account(self, thread_id, last_empty_account=None):
        with self.next_account_lock:
            ...
            while True:
                ...
                if result[-1] in self.used_account_key:
    ...
    def check_available_account_num(self):
        # for k in self.used_account_key:
        available_num = 0
        for name_, password_, k in self.all_account:
            if k not in self.used_account_key:
                available_num+=1
...
class OpenAI_API_inp_Manager_MultiThread:
    def __init__(self, idx_x_list_to_decode, inference_hyper_parameter):
        ...
        for i in range(len(inference_hyper_parameter)):
            ...
            assert 'max_length' in inference_hyper_parameter[i], "{} th inference_hyper_parameter has no max_length"
...
def call_openai_multi_thread(inp_list_to_decode, hyper_parameter, num_threads, use_tqdm, turbo_system_message=None):
    idx_x_list_to_decode = list(enumerate(inp_list_to_decode))
    ...
    for i in range(num_threads):
        thread_list.append(MyThread(i, account_manager, inp_manager, 1, pbar, turbo_system_message))
