...
os.makedirs('fitlog_dir_m_size', exist_ok=True)
# os.makedirs()
fitlog.set_log_dir('fitlog_dir_m_size')
...
def main():
    ...
    if 'retrieval' in args.method:
        ...
        logger.info('demo_pool:{}'.format(len(demo_pool)))

        logger.info('demo_embeddings:{}'.format(demo_embeddings.shape))

        demo_pool_and_embeddings = list(zip(demo_pool,demo_embeddings))

        # args.demo_sample_p
        demo_size = int(args.demo_sample_p * len(demo_pool))
        logger.info({'demo_pool_size':demo_size})
        fitlog.add_hyper({'demo_pool_size':demo_size})
        sampled_demo_pool_and_embeddings = random.Random(args.demo_sampling_seed).sample(demo_pool_and_embeddings, k=demo_size)

        demo_pool = list(map(lambda x:x[0],sampled_demo_pool_and_embeddings))
        demo_embeddings = list(map(lambda x: np.expand_dims(x[1],0), sampled_demo_pool_and_embeddings))
        # np.unsq

        demo_embeddings = np.concatenate(demo_embeddings,axis=0)
    ...
    if 'retrieval' in args.method:
        ...
        @cache_results(_cache_fp='./embeddings_caches/query_embedding_{}_{}_{}_{}_{}_{}_{}_{}'.format(args.dataset,
                                                                                                args.retriever_name.replace(
                                                                                                    '/', '_'),
                                                                                                args.demo_encoding,
                                                                                                args.query_encoding,
    ...
    if args.method in ['lm_retrieval_few_shot_cot', 'lm_retrieval_few_shot',
        ...
        if args.num_demo == 4:
            @cache_results(
                _cache_fp='lm_r_cache/{}/{}_{}_{}_{}_{}_{}'.format(args.dataset, args.retriever_name,
                                                             args.clustered_retrieval, args.model,
                                                             args.demo_pool_path.replace('/', '_'),
                                                                   args.demo_sample_p,
                                                                   args.demo_sampling_seed,
                                                                   ), _hash_param=True)
            def tmp():
            ...
            @cache_results(
                _cache_fp='lm_r_cache/{}/{}_{}_{}_{}_{}_{}_{}'.format(args.dataset, args.retriever_name,
                                                                args.clustered_retrieval, args.model,
...
def parse_arguments():
    ...
    parser.add_argument('--demo_sample_p',type=float)
